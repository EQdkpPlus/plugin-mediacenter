	...
	public function edit(){
		...
		foreach($arrGroups as $gid){
			$this->tpl->assign_block_vars('group_row', array(
				...
				'DD_ADD_ALBUM' 		=> (new hdropdown('perm[ada]['.$gid.']', array('options' => $arrPermissionDropdown, 'value' => (isset($arrPermissions['ada'][$gid]) ? $arrPermissions['ada'][$gid] : -1), 'js' => 'onchange="calculate_permission(\'ada\', '.$gid.', this)"')))->output(),
		...
		if ($id){
			...
			$this->tpl->assign_vars(array(
				...
				'DD_PARENT' 		=> (new hdropdown('parent', array('js'=>'onchange="renew_all_permissions();"', 'options' => $arrCategories, 'value' => $this->pdh->get('mediacenter_categories', 'parent', array($id)))))->output(),
				'DD_PUBLISHED_STATE'=> (new hradio('default_published_state', array('options' => array(0 => $this->user->lang('not_published'), 1 => $this->user->lang('published')), 'value' => $this->pdh->get('mediacenter_categories', 'default_published_state', array($id)))))->output(),
				'R_NOTIFY_UNPUBLISHED' => (new hradio('notify_unpublished', array('value' => ($this->pdh->get('mediacenter_categories', 'notify_on_onpublished', array($id))))))->output(),
				'R_COMMENTS'		=> (new hradio('allow_comments', array('value' => ($this->pdh->get('mediacenter_categories', 'allow_comments', array($id))))))->output(),
				'R_VOTING'			=> (new hradio('allow_voting', array('value' => ($this->pdh->get('mediacenter_categories', 'allow_voting', array($id))))))->output(),
				'DD_LAYOUT_TYPE' 	=> (new hdropdown('layout', array('options' => $this->user->lang('mc_layout_types'), 'value' => $this->pdh->get('mediacenter_categories', 'layout', array($id)))))->output(),
				'DD_MEDIA_TYPE' 	=> (new hmultiselect('types', array('options' => $this->user->lang('mc_types'), 'value' => $this->pdh->get('mediacenter_categories', 'types', array($id)))))->output(),
				'R_PUBLISHED'		=> (new hradio('published', array('value' =>  ($this->pdh->get('mediacenter_categories', 'published', array($id))))))->output(),
				'SPINNER_PER_PAGE'	=> (new hspinner('per_page', array('value' =>  ($this->pdh->get('mediacenter_categories', 'per_page', array($id))), 'max'  => 50, 'min'  => 5,'step' => 5,'onlyinteger' => true)))->output(),
			));
			...
			$this->tpl->assign_vars(array(
				...
				'DD_PUBLISHED_STATE'=> (new hradio('default_published_state', array('options' => array(0 => $this->user->lang('not_published'), 1 => $this->user->lang('published')), 'value' => 1)))->output(),
				'R_NOTIFY_UNPUBLISHED' => (new hradio('notify_unpublished', array('value' => 0)))->output(),
				'R_COMMENTS'		=> (new hradio('allow_comments', array('value' => 1)))->output(),
				'DD_LAYOUT_TYPE' 	=> (new hdropdown('layout', array('options' => $this->user->lang('mc_layout_types'), 'value' => 0)))->output(),
				'DD_MEDIA_TYPE' 	=> (new hmultiselect('types', array('options' => $this->user->lang('mc_types'), 'value' => array(0,1,2))))->output(),
				'R_PUBLISHED'		=> (new hradio('published', array('value' =>  1)))->output(),
				'R_VOTING'			=> (new hradio('allow_voting', array('value' => 1)))->output(),
				'SPINNER_PER_PAGE'	=> (new hspinner('per_page', array('value' => $this->config->get('per_page', 'mediacenter'), 'max'  => 50, 'min'  => 5,'step' => 5,'onlyinteger' => true)))->output(),	
			));
		}
