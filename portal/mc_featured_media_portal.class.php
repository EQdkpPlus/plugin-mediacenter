...
/*+----------------------------------------------------------------------------
  ...
  +--------------------------------------------------------------------------*/
class mc_featured_media_portal extends portal_generic{

	/**
	* Portal path
	*/
	protected static $path = 'mc_featured_media';
	/**
	* Portal data
	*/
/**
	...
	*/
	protected static $data = array(
		...
		'description'	=> 'Displays Featured Media from the MediaCenter',
		'lang_prefix'	=> 'mc_',
		'multiple'		=> true,
	);
	...
	*/
	public function output(){
		...
		$arrTmpMedia = array_unique($arrMedia);
		$arrMedia = array();
		foreach($arrTmpMedia as $intMediaID){
			if($this->pdh->get('mediacenter_media', 'featured', array($intMediaID))){
				$arrMedia[] = $intMediaID;
			}
		}
		...
		foreach($arrMedia as $intMediaID){
			...
			if((int)$arrMediaData['type'] === 0){
				...
				$strOut .= '<div class="mcBoxContent"><h3><a href="'.$strPath.'">'.$this->pdh->get('mediacenter_media', 'name', array($intMediaID)).'</a></h3>
					'.$this->pdh->geth('mediacenter_media', 'type', array($intMediaID)).' - '.$strDate.'<br />'.$strAuthor.' - '.$strCategory.'
					</div>';
				$strOut .= '</div>';
